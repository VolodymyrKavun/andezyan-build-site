...
const DynamicMap = dynamic(() =>
  import('@/components/share/GoogleMap/Map.jsx')
);

const Contacts = () => {
  const neededPhoneProperties = [
    'telFirst',
    'telSecond',
    'telThird',
    'telFourth',
  ...
  const phonesArr = createArrOfNeededObjProperties(
    contactsData,
    neededPhoneProperties
  );

  const neededAdressesProperties = ['physicalAddress', 'registrationAddress'];
  const adressesArr = createArrOfNeededObjProperties(
    contactsData,
    neededAdressesProperties
  );
  ...
  const socLinksArr = createArrOfNeededObjProperties(
    socialData,
    neededSocLinksProperties
  );
    ...
    <section className={styles.container}>
      ...
      <div className={styles.dataWrapper}>
        ...
        <div className={styles.addressesWrapper}>
          <a
            href={adressesArr[0].googleLink}
            target="_blank"
            rel="noopener noreferrer nofollow"
            className={styles.addressContact}
          >
            <svg className={styles.addressSvg}>
              <use
                href="/sprite.svg#icon_navigation"
